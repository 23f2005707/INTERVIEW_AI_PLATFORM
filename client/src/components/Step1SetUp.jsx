...
const Step1SetUp = ({ onStart }) => {
  ...
  const handleUploadResume = async() => {
    // resume file is not exists 
    if(!resumeFile || analyzing) return;
    console.log(resumeFile)
    setAnalyzing(true);

    // append the resumedata into formdata format to send backend. 
  ...
  const handleStart = async() => {
    try{
      ...
      if(userData) {  // update user credits
        dispatch(setUserData({...userData, credits:result.data.creditsLeft}))
      }

      setLoading(false)

      onStart(result.data) // questions data go to second step 
        ...
        <motion.div
          ...
          {/* EXPERIENCE SECTION */}
            ...
            <select value = {mode}
              ...
              {/* When the analysis of resume is not done */}
              {!analysisDone && (
                ...
                className='border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-green-500 hover:bg-green-50 transition'>
                  ...
                  <p className='text-gray-600 font-medium'>
                    {resumeFile ? resumeFile.name : "Click to Upload file (Optional)"}
                  </p>


                  {/* RESUME FILE EXISTS  and analyze the resume*/}
                  {resumeFile && (
                    <motion.button
                      whileHover={{scale: 1.02}}

                      // apply handleupload resume Function
              ...
              {/* ANALYSIS DONE */}
              {analysisDone && (
                <motion.div className='bg-gray-50 border-gray-200 rounded-xl p-5 space y-4'>
              ...
              <motion.button 
                ...
                whileHover={{scale: 1.03}}
                whileTap={{ scale: 0.95}}
                className='w-full disabled:bg-gray-600 bg-green-600 hover:bg-green-700 text-wite py-3 rounded-full text-lg font-semibold transition duration-300 shadow-md'>
